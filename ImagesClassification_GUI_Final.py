"""
Modernized AI Image Classifier GUI (1000 ImageNet Classes)

- Uses pretrained ResNet-18 from torchvision
- Returns the best-matching class from all 1000 ImageNet categories
- Shows top-5 predictions with confidence percentages
- Classifies single images or an entire folder (with progress %, ETA)
- Modernized UI using ttk widgets and custom styling
- Thread-safe updates; keeps UI responsive during work
...
class ModernClassifierApp(tk.Tk):
    def __init__(self):
        ...
        self.style.configure("Card.TLabel", background=self.card, foreground=self.text, font=self.font_normal)
        self.style.configure("Heading.TLabel", background=self.surface, foreground=self.text, font=self.font_heading)
        self.style.configure("CardHeading.TLabel", background=self.card, foreground=self.text, font=self.font_heading)
        self.style.configure("Muted.TLabel", background=self.surface, foreground=self.muted, font=self.font_normal)
        self.style.configure("CardMuted.TLabel", background=self.card, foreground=self.muted, font=self.font_normal)
        self.style.configure("Mono.TLabel", background=self.surface, foreground=self.text, font=self.font_mono)
        self.style.configure("CardMono.TLabel", background=self.card, foreground=self.text, font=self.font_mono)
        self.style.configure("TButton", font=self.font_normal, padding=6)
        self.style.configure("Accent.TButton", foreground="#FFFFFF", background=self.accent)
        self.style.map("Accent.TButton", background=[("active", self.primary)], foreground=[("active", "#FFFFFF")])
        self.style.configure("Primary.TButton", foreground="#FFFFFF", background=self.primary)
        self.style.configure("Danger.TButton", foreground="#FFFFFF", background="#EF4444")
    ...
    def _build_ui(self):
        ...
        self.classify_folder_btn = ttk.Button(controls_card, text="Classify Folder", style="Primary.TButton", command=self._start_folder)
